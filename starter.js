...
const mappedColors = colors.map((elem) => `${elem} is better than pink`)
...
const placesThatStartWithA = places.filter((place) => place[0] === 'A' || place[0] === 'a') // checking for both upper and lower case letters. you probably won't have to worry about this for the assessment
...
const productOfArray = numbers => {
    ...
    //     total += numbers[i]
    //     console.log(`Total: ${total}, Current: ${numbers[i]}`)
    // }

    // return total

    return numbers.reduce((accumulative, current) => accumulative + current)
}
...
//     total2 += expenses[i].amount
// }

// console.log(budget - total2)